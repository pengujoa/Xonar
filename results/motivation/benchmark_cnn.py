...
_DEFAULT_PARAMS = {
    ...
    'data_format':
        ParamSpec('string', 'NHWC', #cyshin modified NCHW -> NHWC
...
def benchmark_one_step(sess,
  ...
  if trace_filename and step == 10:
    ...
    for partition_graph_def in run_metadata.partition_graphs:
      path, filename = os.path.split(params.graph_file)
      filename = str(index) + "_" + filename
      index = index + 1
      as_text = filename.endswith('txt')
      tf.train.write_graph(partition_graph_def, path, filename, as_text)      
